...
MIN_24H_CHANGE = 5.0
MAX_1H_CHANGE = -1.5   # Junaman paling sikit
MIN_1H_CHANGE = -5.0   # Junaman paling maksimum (elak rugpull)
...
# =====================================================================
# 2. LIVE API FETCHERS (DEXSCREENER & COINGECKO)
# =====================================================================
    ...
    try:
        ...
        if res.get('pairs'):
            ...
            return {
                ...
                'chain_raw': chain_id, # Untuk URL Dexscreener
...
def calculate_real_technicals(coin_id, current_price):
    """Menyedut data OHLC 7 Hari dari CoinGecko untuk kiraan Fibo & RSI"""
    try:
        # Panggil data 7 hari (interval kebiasaannya 1 jam di CG)
        ...
        elif not in_zone: verdict = "WAITING FOR PULLBACK 🟡"
...
def execute_confluence_protocol(dex_data, coin_id):
    """Menyemak kelulusan 5 Fasa (Dipendekkan ke dalam logs)"""
    # FASA 1 & 2: MCap, Liq, Volume
    ...
    # FASA 3: Momentum (Buy > Sell, 24H > 5%, 1H -1.5% to -5.0%)
...
# =====================================================================
# 5. BROADCAST UI (DYNAMIC INLINE BUTTONS FIX)
# =====================================================================
def send_signal(coin_info, dex_data, rsi_txt, fibo_txt, verdict, target_chat_id=VIP_CHANNEL_ID):
    ...
    # Dynamic Chain URL untuk Dexscreener
...
"""
    ...
    markup.add(
        InlineKeyboardButton("🐦 Twitter Search", url=f"https://twitter.com/search?q=%24{sym}"),
...
# =====================================================================
# 6. ENJIN PENGIMBAS SEBENAR (DENGAN VERBOSE LOGGING)
# =====================================================================
def run_live_scan(categories):
    for cat in categories:
        ...
        if not coins:
            print(f"   [-] Tiada data CoinGecko. Skip...")
            continue
        ...
        for coin in coins:
            ...
            time.sleep(0.5) # Rehat sikit
            
            if not dex_data: 
                print("     [X] Tiada data rantaian DexScreener.")
                continue
        ...
        time.sleep(2) # Elak Rate Limit CG
...
def cmd_ca(message):
    try:
        ...
        if dex_data:
            # Bypass untuk Manual CA (Tunjuk data je)
            url = f"https://api.coingecko.com/api/v3/search?query={dex_data['symbol']}"
            res = requests.get(url).json()
            cg_id = res['coins'][0]['id'] if res.get('coins') else None
            
            if cg_id: rsi, fibo, ver, _, _ = calculate_real_technicals(cg_id, dex_data['price_usd'])
            else: rsi, fibo, ver = "RSI N/A", "Fibo N/A", "MANUAL ANALYZE"
            ...
            bot.reply_to(message, "❌ Data DexScreener gagal ditarik.")
